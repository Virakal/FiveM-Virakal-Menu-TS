...
export default class VehicleHandler implements Handler {
	...
	constructor(trainer: Trainer) {
		...
		RegisterNuiCallback(
			'vehcustomsecondary',
			this.onVehCustomSecondary.bind(this),
		);
		...
		RegisterNuiCallback(
			'vehtyresmokecolour',
			this.onVehTyreSmokeColour.bind(this),
		);
	...
	async onVehLivery(data: NuiData, cb: NuiCallback): Promise<NuiCallback> {
		...
		if (liveryCount === -1) {
			...
			console.log(
				`No basic liveries supported, ${modLiveryCount} mod liveries instead.`,
			);
			...
			notify(
				`~r~${getVehicleName(vehicle)} does not have enough liveries to set to ${liveryId}!`,
			);
		} else {
			...
			if (name) {
				console.log(
					`~g~Set ${getVehicleName(vehicle)} livery to ${name} (${liveryId}/${liveryCount - 1})`,
				);
			} else {
				console.log(
					`~g~Set ${getVehicleName(vehicle)} livery to ${liveryId}/${liveryCount - 1}`,
				);
			}
		}
	...
	onVehTint(data: NuiData, cb: NuiCallback): NuiCallback {
		const { action } = data;
		const vehicle = GetVehiclePedIsUsing(PlayerPedId());
	...
	onRainbowSpeed(data: NuiData, cb: NuiCallback): NuiCallback {
		...
		if (speed > 0) {
			...
			notify(
				`~r~Error setting rainbow speed! Set to ${this.rainbowSpeed * 100}`,
			);
		}
	...
	async onVehNeon(data: NuiData, cb: NuiCallback): Promise<NuiCallback> {
		...
		if (action === 'input') {
			...
			const match = action.match(
				/(?:^(?<set>on|off)(?<light>\d)$|(?<colour>^\d+,\d+,\d+$))/,
			);
	...
	private async changeCustomColours(
		vehicle: number,
		action: string,
