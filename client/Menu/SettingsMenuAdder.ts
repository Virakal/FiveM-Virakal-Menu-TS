...
export default class SettingsMenuAdder {
	...
	getDefaultRadioMenu(): MenuItem[] {
		...
		if (config.has('DefaultRadioStation')) {
			currentDefaultStation =
				Number.parseInt(config.get('DefaultRadioStation'), 10) ?? -1;
		}
		...
		const radioIds = new Array(CLEAN_STATION_COUNT).fill(true);
		const radioStationsArray: string[] = radioIds.map((_, i) =>
			getRadioStationName(i),
		);
		...
		for (const [i, name] of radioStations) {
			const maybeCurrentSuffix =
				currentDefaultStation === i ? ' (Current)' : '';
	...
	private getTimeMenu(): MenuItem[] {
		const menu: MenuItem[] = [];
		const namedTimes = new Map<string, number>(
			Object.entries({
				Morning: 6,
				Midday: 12,
				Evening: 18,
