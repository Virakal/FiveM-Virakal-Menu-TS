...
export default class TeleportHandler implements Handler {
	...
	onTelePlayer(data: NuiData, cb: NuiCallback): NuiCallback {
		...
		console.log(
			`Teleporting ${playerServerId} (${playerClientId}) to ${otherServerId} (${otherClientId}).`,
		);
	...
	onTeleLastCar(data: NuiData, cb: NuiCallback): NuiCallback {
		...
		if (currentVehicle) {
			errorMessage =
				"You can't teleport to a new vehicle while you're already in a vehicle.";
		} else if (!lastVehicle) {
	...
	async onTeleWaypoint(data: NuiData, cb: NuiCallback): Promise<NuiCallback> {
		...
		teleportToGroundHeight(
			...
			waypointPosition,
			true,
			TP_TO_WAYPOINT_ADDITIONAL_HEIGHT,
		);
