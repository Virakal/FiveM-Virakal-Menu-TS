...
class Garage {
	...
	async loadVehicle(slot: string): Promise<number> {
		...
		console.log(
			`Loaded from ${configKey}. Name: ${info.displayName} Model: ${info.model}`,
		);
		...
		switch (version) {
			...
			default:
				throw new SyntaxError(
					'Failed to deserialise garage vehicle - could not determine the version.',
				);
		}
	}
	...
	toModString(vehicle: number, _mods: ModList): string {
		...
		if (GetIsVehicleSecondaryColourCustom(vehicle)) {
			modList.CustomSecondary =
				GetVehicleCustomSecondaryColour(vehicle).join(',');
		} else {
		...
		for (const value of Object.values(VehicleNeonLight)) {
			...
			modList[`NeonEnabled${value}`] = IsVehicleNeonLightEnabled(
				vehicle,
				value as number,
			)
				? 'true'
	...
	private async applyModString(vehicle: number, modString: string) {
		...
		if ('CustomSecondary' in modList) {
			...
			const colour = modList.CustomSecondary.split(',').map((x) =>
				Number.parseInt(x.trim()),
			);
		...
		if ('PrimaryColour' in modList) {
			...
			SetVehicleColours(
				vehicle,
				Number.parseInt(modList.PrimaryColour),
				GetVehicleColours(vehicle)[1],
			);
		}
		...
		if ('SecondaryColour' in modList) {
			...
			SetVehicleColours(
				vehicle,
				GetVehicleColours(vehicle)[0],
				Number.parseInt(modList.SecondaryColour),
			);
		}
		...
		if ('PearlescentColour' in modList) {
			...
			SetVehicleExtraColours(
				vehicle,
				Number.parseInt(modList.PearlescentColour),
				GetVehicleExtraColours(vehicle)[1],
			);
		}
		...
		if ('PlateStyle' in modList) {
			...
			SetVehicleNumberPlateTextIndex(
				vehicle,
				Number.parseInt(modList.PlateStyle),
			);
		}
		...
		if ('RimColour' in modList) {
			...
			SetVehicleExtraColours(
				vehicle,
				GetVehicleExtraColours(vehicle)[0],
				Number.parseInt(modList.RimColour),
			);
		}
		...
		if ('DashboardColour' in modList) {
			...
			SetVehicleDashboardColour(
				vehicle,
				Number.parseInt(modList.DashboardColour),
			);
		}

		if ('NeonColour' in modList) {
			console.log(`Setting neon colour to ${modList.NeonColour}`);
			const colour = modList.NeonColour.split(',').map((x) =>
				Number.parseInt(x.trim()),
			);
		...
		for (const [key, value] of Object.entries(modList)) {
			...
			console.log(
				`Setting mod ${modType} (${addSpacesToCamelCase(VehicleModType[modType])}) (${await getModTypeName(vehicle, modType)}) to ${value} (${await getModName(vehicle, modType, index)})`,
			);
			SetVehicleMod(
				vehicle,
				modType,
				index,
				GetVehicleModVariation(modType, index),
			);
		}
	}
}
